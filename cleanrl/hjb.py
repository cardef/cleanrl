...
try: from torch.func import grad, vmap, jacrev; TORCH_FUNC_AVAILABLE = True; print("Imported grad, vmap, jacrev from torch.func")
except ImportError:
    try: from functorch import grad, vmap, jacrev; TORCH_FUNC_AVAILABLE = True; print("Imported grad, vmap, jacrev from functorch")
    except ImportError: print("WARNING: torch.func / functorch not available. HJB calculations will be skipped."); TORCH_FUNC_AVAILABLE = False
try: import torchode as to; TORCHODE_AVAILABLE = True; print("Imported torchode.")
except ImportError: print("FATAL: torchode not found. Exiting."); TORCHODE_AVAILABLE = False; exit()
...
@dataclass
class Args:
    exp_name: str = os.path.basename(__file__)[: -len(".py")] + "_hjb_vi_ode_inferred_cost" # VI=ValueIteration-like
    ...
    env_id: str = "HalfCheetah-v4"; total_timesteps: int = 1000000
...
class DynamicModel(nn.Module): # (Same)
    def __init__(self, obs_dim, action_dim, dt: float, device: torch.device):
        super().__init__(); self.ode_func = ODEFunc(obs_dim, action_dim); self.dt = dt; self.device = device;
        if not TORCHODE_AVAILABLE: raise ImportError("torchode not found.");
        self.term=to.ODETerm(self.ode_func,with_args=True); self.step_method=to.Euler(term=self.term); self.step_size_controller=to.FixedStepController(); self.adjoint=to.AutoDiffAdjoint(step_method=self.step_method, step_size_controller=self.step_size_controller);
...
def train_model_epoch(model, optimizer, train_loader, device, writer, epoch, global_step, model_name, is_dynamic_model): # (Same)
    model.train(); total_loss=0.0; num_batches=0;
    for batch_idx,batch_data in enumerate(train_loader): obs_norm, actions, targets_norm = [d.to(device) for d in batch_data];
    if is_dynamic_model: preds_norm = model(obs_norm, actions)
    else: raise ValueError("train_model_epoch called without dynamic model flag") # No reward model
...
def validate_model(model, val_loader, device, writer, global_step, model_name, is_dynamic_model): # (Same)
    model.eval(); all_preds_norm=[]; all_targets_norm=[];
    with torch.no_grad():
        for batch_data in val_loader: obs_norm, actions, targets_norm = [d.to(device) for d in batch_data];
        if is_dynamic_model: preds_norm = model(obs_norm, actions)
        else: raise ValueError("validate_model called without dynamic model flag") # No reward model
...
if __name__ == "__main__":
    ...
    if args.track:
        try:
            ...
            print("WARNING: wandb not installed."); args.track=False
    ...
    norm_envs = VecNormalize(envs, gamma=args.gamma, norm_obs=True, norm_reward=True, clip_obs=10.0, clip_reward=10.0); print("VecNormalize enabled.")
    ...
    try:
        ...
        if args.ctrl_cost_weight is not None:
             print(f"Warning: Overriding inferred ctrl_cost_weight with args.ctrl_cost_weight={args.ctrl_cost_weight}")
        ...
        if args.ctrl_cost_weight is None:
             ...
             print(f"Warning: Could not detect env ctrl_cost_weight. Using default: {args.ctrl_cost_weight}")
        ctrl_cost_weight = args.ctrl_cost_weight # Use the one from args (might be default)
    ...
    critic_optimizer = optim.AdamW(critic.parameters(), lr=args.learning_rate); # Use main LR
    ...
    if args.use_hjb_loss and TORCH_FUNC_AVAILABLE:
        try:
            def compute_scalar_value_critic(single_obs_tensor):
                if single_obs_tensor.dim() == 1: single_obs_tensor = single_obs_tensor.unsqueeze(0)
    ...
    def get_f1(s_norm_batch):
        ...
        with torch.no_grad(): f1_pred = dynamic_model.ode_func(0, s_norm_batch, zero_actions)
        return f1_pred
    ...
    if TORCH_FUNC_AVAILABLE:
        try: compute_jac_f_a = jacrev(func_for_jac, argnums=2) # Jacobian wrt action (arg 2)
        except Exception as e: print(f"WARN: jacrev failed: {e}. Cannot compute f2."); TORCH_FUNC_AVAILABLE=False # Disable HJB if jacrev fails

    def get_f2_transpose(s_norm_batch):
        if compute_jac_f_a is None: return None
        try:
            ...
            def compute_jac_for_single_s(s_single, a_single): # Pass action too
                 ...
                 jacobian_matrix = compute_jac_f_a(torch.tensor(0.0), s_batch, a_batch) # Pass t=0
                 return jacobian_matrix.squeeze(0) # Remove added batch dim -> [obs_dim, action_dim]
            ...
            return f2_transpose
        except Exception as e: print(f"ERROR computing f2 Jacobian: {e}"); return None
    ...
    def calculate_a_star(dVdx_norm, f2_transpose):
        ...
        a_star = (-1.0 / (2.0 * ctrl_cost_weight)) * torch.bmm(f2_transpose, dVdx_col).squeeze(-1)
        return a_star
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step < args.learning_starts:
            ...
            with torch.no_grad():
                ...
                actions_star = torch.zeros(args.num_envs, action_dim).to(device) # Default if calculation fails
        ...
        next_obs_norm = next_obs_norm.astype(np.float32); rewards_norm = rewards_norm.astype(np.float32)
        ...
        for info in final_infos:
             if info and "episode" in info:
                  episode_info=info['episode']; return_val=float(episode_info['r']); length_val=int(episode_info['l']);
                  print(f"GStep={global_step}, EpReturn(Norm)={return_val:.2f}, EpLen={length_val}")
                  writer.add_scalar("charts/episodic_return_norm",return_val,global_step); writer.add_scalar("charts/episodic_length",length_val,global_step); break

        # Store RAW data in replay buffer
        raw_obs = norm_envs.get_original_obs(); raw_reward = norm_envs.get_original_reward();
        ...
        rb.add(raw_obs, real_next_obs_raw, actions, raw_reward, dones_combined_np, infos) # Pass combined done
        ...
        if global_step > args.learning_starts and global_step % args.model_train_freq == 0:
             ...
             obs_raw_np=buffer_data_raw_tensors.observations.cpu().numpy(); next_obs_raw_np=buffer_data_raw_tensors.next_observations.cpu().numpy(); actions_np=buffer_data_raw_tensors.actions.cpu().numpy(); dones_term_only_np=buffer_data_raw_tensors.dones.cpu().numpy();
             obs_norm_np=norm_envs.normalize_obs(obs_raw_np); next_obs_norm_np=norm_envs.normalize_obs(next_obs_raw_np);
             obs_norm_t=torch.tensor(obs_norm_np,dtype=torch.float32).to(device); next_obs_norm_target_t=torch.tensor(next_obs_norm_np,dtype=torch.float32).to(device); actions_t=torch.tensor(actions_np,dtype=torch.float32).to(device); dones_term_only_t=torch.tensor(dones_term_only_np,dtype=torch.float32).to(device);
             # --- Dynamic Model ---
             non_terminal_mask_dyn = dones_term_only_t.squeeze(-1) == 0
             dyn_obs_t=obs_norm_t[non_terminal_mask_dyn]; dyn_acts_t=actions_t[non_terminal_mask_dyn]; dyn_targets_t=next_obs_norm_target_t[non_terminal_mask_dyn];
             if len(dyn_obs_t) < 2: print("Warn:Not enough samples for dyn model."); dynamic_model_accurate=False
                 ...
                 for epoch in range(args.model_max_epochs):
                     ...
                     if (epoch+1)%args.model_validation_freq==0:
                         ...
                         if val_loss<best_val_loss-args.model_val_delta: best_val_loss=val_loss;patience_counter=0;best_dyn_state_dict=copy.deepcopy(dynamic_model.ode_func.state_dict()); # Save ODEFunc state
        ...
        if global_step > args.learning_starts:
            ...
            if proceed_with_update:
                ...
                obs_norm_np=norm_envs.normalize_obs(obs_raw_np); rewards_norm_np=norm_envs.normalize_reward(rewards_raw_np.reshape(-1)).reshape(-1,1);
                mb_obs = torch.tensor(obs_norm_np, dtype=torch.float32).to(device)
                mb_actions = torch.tensor(actions_np, dtype=torch.float32).to(device) # Action from buffer
                mb_rewards = torch.tensor(rewards_norm_np, dtype=torch.float32).to(device).squeeze(-1) # Normalized reward [batch]
                ...
                mb_obs_critic = mb_obs.clone().requires_grad_(True) # Need grad for dVdx
                ...
                if non_terminations_mask.any() and args.use_hjb_loss and compute_value_grad_func is not None:
                    ...
                    actions_buffer_non_term = mb_actions[non_terminations_mask] # Action actually taken
                    rewards_buffer_non_term = mb_rewards[non_terminations_mask] # Reward actually received (normalized)
                    ...
                    try:
                        ...
                        if f2_T_non_term is not None:
                            a_star_non_term = calculate_a_star(dVdx_non_term, f2_T_non_term)
                            ...
                            r_state_approx = rewards_buffer_non_term + ctrl_cost_weight * torch.sum(actions_buffer_non_term**2, dim=1) # Assumes a_buffer is raw action scale? No, should use normalized? Let's assume quadratic cost applies to raw actions. Need raw actions here.
                            ...
                            with torch.no_grad(): f_buffer_action = dynamic_model.ode_func(0, obs_non_term, actions_buffer_non_term)
                            hjb_residual = (rewards_buffer_non_term + torch.einsum("bi,bi->b", dVdx_non_term, f_buffer_action)) - rho * v_non_term
        ...
        if global_step > 0 and global_step % 1000 == 0: sps = int(global_step / (time.time() - start_time)); print(f"GStep: {global_step}, SPS: {sps}"); writer.add_scalar("charts/SPS", sps, global_step)
    ...
    if args.save_model:
        ...
        if TORCH_FUNC_AVAILABLE:
             try:
                 ...
                 def eval_func_for_jac(t, s, a): return eval_dynamic_model.ode_func(t, s, a)
                 ...
                 def eval_compute_jac_for_single_s(s_single):
                      ...
                      jacobian_matrix=eval_compute_jac_f_a(torch.tensor(0.0),s_batch,a_zeros_single); return jacobian_matrix.squeeze(0)
                 ...
                 def eval_calculate_a_star(dVdx_norm, f2_transpose):
                     ...
                     a_star = (-1.0/(2.0*ctrl_cost_weight))*torch.bmm(f2_transpose,dVdx_col).squeeze(-1)
                     return a_star
             except Exception as e: print(f"Eval grad/jac func failed: {e}")
            ...
            while not done and num_steps<max_steps:
                with torch.no_grad():
                    ...
                    if eval_compute_value_grad_func is not None and eval_get_f2_transpose is not None and eval_calculate_a_star is not None:
                        try:
                            obs_tensor_grad = obs_tensor.clone().requires_grad_(True) # Need requires_grad? No, grad func handles it.
                            ...
                            if f2_T is not None:
                                ...
                                if action_star is not None:
                                    action = action_star.cpu().numpy().clip(action_space.low, action_space.high)
                        except Exception as e: print(f"Eval action calc failed: {e}") # Use default random if fails
                    else: # Use random action if grad funcs failed
                         action = np.array([eval_norm_envs.action_space.sample() for _ in range(eval_norm_envs.num_envs)])
