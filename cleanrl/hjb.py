...
# --- Reward Model ---
# <<< Re-introduced RewardModel >>>
class RewardModel(nn.Module):
    def __init__(self, obs_dim, action_dim):
        super().__init__()
        hidden_size=128 # Can be smaller than dynamics model
        self.net=nn.Sequential(
...
def train_model_epoch(model, optimizer, train_loader, device, writer, epoch, global_step, model_name, is_dynamic_model):
    model.train(); total_loss=0.0; num_batches=0;
    for batch_idx,batch_data in enumerate(train_loader):
        obs_norm, actions, targets_norm = [d.to(device) for d in batch_data];
        # <<< Updated: Handle reward model training target >>>
        if is_dynamic_model:
            preds_norm = model(obs_norm, actions) # Predicts next_obs_norm
        else: # Reward model
            preds_norm = model(obs_norm, actions) # Predicts reward_norm
...
def validate_model(model, val_loader, device, writer, global_step, model_name, is_dynamic_model):
    model.eval(); all_preds_norm=[]; all_targets_norm=[];
    with torch.no_grad():
        for batch_data in val_loader:
            obs_norm, actions, targets_norm = [d.to(device) for d in batch_data];
            # <<< Updated: Handle reward model prediction >>>
            if is_dynamic_model: preds_norm = model(obs_norm, actions)
            else: preds_norm = model(obs_norm, actions)
...
if __name__ == "__main__":
    ...
    # <<< Re-introduced RewardModel and optimizer >>>
    reward_model = RewardModel(obs_dim, action_dim).to(device);
    reward_optimizer = optim.AdamW(reward_model.parameters(), lr=args.reward_learning_rate);
    ...
    if args.use_hjb_loss and TORCH_FUNC_AVAILABLE:
        try:
            ...
            # Grad R(s, a) w.r.t a
            def reward_model_wrapper_for_grad(s, a): # Takes s, a -> R(s,a)
                if s.dim() == 1: s = s.unsqueeze(0)
                if a.dim() == 1: a = a.unsqueeze(0)
                return reward_model(s, a).squeeze() # Ensure scalar output if batch=1
            compute_reward_grad_func = grad(reward_model_wrapper_for_grad, argnums=1) # Grad w.r.t action (arg 1)

            # Hessian R(s, a) w.r.t a
            compute_reward_hessian_func = hessian(reward_model_wrapper_for_grad, argnums=1) # Hessian w.r.t action (arg 1)

            # Jacobian f(s, a) w.r.t a
            func_for_jac = dynamic_model.ode_func
            compute_jac_f_a = jacrev(func_for_jac, argnums=2) # Jacobian w.r.t action (arg 2)
            def get_f2_transpose(s_norm_batch): # Helper to handle vmap/batching for jacobian
    ...
    # --- Helper Functions for HJB Calculation ---
    ...
    def calculate_a_star_quad_approx(dVdx_norm, f2_transpose, c1, c2_reg):
        ...
        dVdx_col = dVdx_norm.unsqueeze(-1) # [b, o, 1]
        f2T_dVdx = torch.bmm(f2_transpose, dVdx_col).squeeze(-1) # [b, a]
        term1 = c1 + f2T_dVdx # [b, a]
        # Solve batch linear system: c2_reg @ a* = -term1
        # Reshape term1 to be [b, a, 1] for solve
        try:
            # Use torch.linalg.solve for potentially better stability/efficiency
            a_star = torch.linalg.solve(c2_reg, -term1.unsqueeze(-1)).squeeze(-1) # [b, a]
        except torch._C._LinAlgError as e:
             print(f"WARN: Hessian inversion failed during a* calc: {e}. Using pseudo-inverse.")
             try:
                  c2_reg_pinv = torch.linalg.pinv(c2_reg) # Pseudo-inverse [b, a, a]
    ...
    for global_step in range(args.total_timesteps):
                ...
                if compute_value_grad_func is not None and compute_reward_grad_func is not None \
                   and compute_reward_hessian_func is not None and compute_f_jac_func is not None:
                    try:
                        ...
                        # Need c1 = -grad_a R(s, 0) and c2 = -hess_aa R(s, 0)
                        zero_actions_obs = torch.zeros_like(actions_star) # Actions at a=0
                        c1 = -vmap(compute_reward_grad_func)(obs_tensor, zero_actions_obs) # [b, a]
                        c2 = -vmap(compute_reward_hessian_func)(obs_tensor, zero_actions_obs) # [b, a, a]
                        # Regularize Hessian c2
                        c2_reg = c2 + torch.eye(action_dim, device=device) * args.hessian_reg
                        actions_star_calc = calculate_a_star_quad_approx(dVdx, f2_T, c1, c2_reg)
        ...
        # Log real env returns (using raw rewards now)
        ...
        if global_step > args.learning_starts and global_step % args.model_train_freq == 0:
             ...
             # Normalize states, keep rewards raw for reward model target
             ...
             # <<< Target for reward model is RAW reward >>>
             rewards_raw_target_t=torch.tensor(rewards_raw_np,dtype=torch.float32).to(device).squeeze(-1); # Target is raw reward [batch]
             dones_term_only_t=torch.tensor(dones_term_only_np,dtype=torch.float32).to(device);

             # --- Dynamic Model Training/Validation ---
             # (Uses normalized states dyn_obs_t, dyn_acts_t -> dyn_targets_t)
             ...
             # --- Reward Model ---
             # <<< Train RewardModel on normalized state/action -> raw reward >>>
             rew_obs_t=obs_norm_t; rew_acts_t=actions_t; rew_targets_t=rewards_raw_target_t.squeeze(-1); # Target is RAW reward
             if len(rew_obs_t)<2: print("Warn:Not enough samples for rew model."); reward_model_accurate=False # Use separate accuracy flag maybe? Or just use loss?
             else:
                 ...
                 for epoch in range(args.model_max_epochs):
                     final_model_epoch=epoch; train_loss=0; num_train_batches=0;
                     for _ in range(args.model_updates_per_epoch): epoch_train_loss=train_model_epoch(reward_model,reward_optimizer,train_loader,device,writer,epoch,global_step,"reward",False); train_loss+=epoch_train_loss; num_train_batches+=1; # Pass is_dynamic=False
                     if (epoch+1)%args.model_validation_freq==0:
                         # Validate reward model against RAW reward targets
                 ...
                 # reward_model_accurate=(validation_loss_reward <= args.reward_train_threshold); # Accuracy threshold less meaningful for raw rewards
                 reward_model_accurate = True # Assume reward model is usable if trained
            ...
            if proceed_with_update:
                ...
                if non_terminations_mask.any() and args.use_hjb_loss and compute_value_grad_func is not None \
                   ...
                   and compute_f_jac_func is not None:
                    ...
                    # Calculate HJB components using normalized state
                    try:
                        ...
                        # Calculate Reward Gradients/Hessians at a=0
                        zero_actions_non_term = torch.zeros(obs_non_term.shape[0], action_dim, device=device)
                        ...
                        if f2_T_non_term is not None and c1 is not None and c2_reg is not None:
                            # Calculate UNCLAMPED optimal action a*(s_norm)
                            a_star_non_term = calculate_a_star_quad_approx(dVdx_non_term, f2_T_non_term, c1, c2_reg)
                            ...
                            if a_star_non_term is not None:
                                ...
                                with torch.no_grad():
                                    ...
                                    # Need R(s, a*)
                                    r_star_non_term = reward_model(obs_non_term, a_star_non_term) # Predicted reward (raw scale)
                ...
                # Retain graph only if dVdx is needed for actor update later? No, actor removed.
    ...
    if args.save_model:
        ...
        # <<< Re-add reward model loading for eval if needed by H? Not directly by a* >>>
        ...
        if TORCH_FUNC_AVAILABLE:
             try:
                 ...
                 # Need to load eval_reward_model first if using it
                 ...
                 def eval_calculate_a_star_quad_approx(dVdx_norm, f2_transpose): # Removed c1, c2_reg args
                    # <<< Recalculate c1, c2 inside using eval models >>>
                    ...
                    if eval_compute_reward_grad_func is None or eval_compute_reward_hessian_func is None:
                        ...
                        return None
                    s_norm_batch = # Need the state batch here! Pass it in.
        ...
        for seed in eval_seeds:
            ...
            while not done and num_steps<max_steps:
                with torch.no_grad():
                    ...
                    if eval_compute_value_grad_func is not None and eval_get_f2_transpose is not None and eval_calculate_a_star_quad_approx is not None: # <<< Check correct function >>>
                        try:
                            dVdx = vmap(eval_compute_value_grad_func)(obs_tensor)
                            f2_T = eval_get_f2_transpose(obs_tensor)
                            # <<< Need c1, c2 for eval a* calculation >>>
                            # This requires reward model and its derivatives during eval!
                            # Simplification: For EVALUATION ONLY, maybe use the simpler a* = -1/(2C) f2^T dVdx^T ?
                            # Let's use the simpler one assuming C=ctrl_cost_weight for eval.
                            if f2_T is not None:
                                dVdx_col = dVdx.unsqueeze(-1)
                                action_star_unclamped = (-1.0/(2.0*ctrl_cost_weight))*torch.bmm(f2_T, dVdx_col).squeeze(-1)
